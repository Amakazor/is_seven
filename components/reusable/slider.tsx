...
export default function Slider(props: sliderProps) {
    ...
    const getValueIndexFromEvent = (position: number) => {
        ...
        return Math.max(0, Math.min(props.Values.length - 1, Math.round((position - boundingBox.left) / (boundingBox.width / (props.Values.length - 1)))));
    }
    ...
    const onKeyPress = (event: React.KeyboardEvent) => {
        ...
        switch (event.key) {
            ...
            case 'ArrowRight':
            case 'ArrowUp':
                if (props.activeIndex < props.Values.length - 1) setState(props.activeIndex + 1);
                break;
        }
    ...
    const aria = {
        ...
        'aria-valuemax': props.isNumeric ? props.Values[props.Values.length - 1] as number : props.Values.length - 1,
        'aria-valuenow': props.isNumeric ? props.Values[props.activeIndex] as number : props.activeIndex,
        'aria-valuetext': props.isNumeric ? null : props.Values[props.activeIndex].toString()
    }
    ...
    return (
        <SliderContainer
            ...
            {...aria}
            title={props.isNumeric ? aria['aria-valuenow'].toString() : aria['aria-valuetext']}>
            <Track  ref={trackRef} {...props}>
                <Tack tabIndex={0} {...props} count={count}  onKeyDown={onKeyPress}/>
                {(props.labelActive && !props.labelAll) 
                    && <ActiveLabel {...props} count={count}>{props.Values[props.activeIndex]}</ActiveLabel>}
                {props.Values.map((value: string | number, index: number) => (props.labelAll || index == 0 || index == props.Values.length - 1) && (
                    <Label {...props} isActive={index == props.activeIndex && props.labelAll && props.labelActive} count={count} index={index} key={index}>
                        {value}
                    </Label>
                ))}
            </Track>
