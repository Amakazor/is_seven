...
export default function NumberSlider(props: slidingNumberSelectProps) {
    const getValues = (min: number, max: number, step: number): number[] => {
        if (step == 0 || Math.abs(step) < Number.EPSILON * 10)
            throw new RangeError(`Value of 'step' cannot be equal to zero.`);
        ...
        if (!isStepValid(amount, step))
            throw new RangeError(
                `Value ${step} is not a valid devisor for value between ${min} and ${max}.`,
            );

        return Array(amount / step + 1)
            .fill(0)
            .map((_, index) => Number.parseFloat((min + index * step).toFixed(2)));
    };

    const isStepValid = (amount: number, step: number): boolean => {
        const modulo = amount % step;
        return (
            Math.abs(modulo) < Number.EPSILON * 10 ||
            Math.abs(modulo) - step < Number.EPSILON * 10
        );
    };
    ...
    return (
        <Slider
            {...props}
            Values={getValues(props.numberMin, props.numberMax, props.numberStep)}
            isNumeric={true}
        />
    );
}
